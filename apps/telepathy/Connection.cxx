     ...
     nextHandleId(1)
{
   ...
   connect(&(getConversationManager()), SIGNAL(onMessageReceived(const resip::SipMessage&)),
...
tr::Connection::createChannel(const QVariantMap &request, Tp::DBusError *error)
{
   ...
   if(channelType == TP_QT_IFACE_CHANNEL_TYPE_CALL) {
	 ...
	 TextChannelPtr textChannel = tr::TextChannel::create(&(getConversationManager()), baseChannel.data(), selfHandle(), mHandles[selfHandle()]);
